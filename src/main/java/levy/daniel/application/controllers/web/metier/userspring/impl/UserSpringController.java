...
public class UserSpringController {
	...
	/**
	 * "numeroPageUserSpring".
	 */
	public static final String NUMEROPAGEUSERSPRING = "numeroPageUserSpring";
	
	/**
	 * "sizeUserSpring".
	 */
	public static final String SIZEUSERSPRING = "sizeUserSpring";
	
	/**
	 * IUserSpringService injecté par SPRING.
	 */
	@Autowired
	...
	/**
	 * IRoleService injecté par SPRING.
	 */
	@Autowired
	...
	/**
	 * Execute une recherche rapide de la liste paginée 
	 * des objets métier à chaque
	 * lancement de l'action
	 * <code>"/UserSpringController/rechercherRapidePagineUserSpring"</code>
	 * .<br/>
	 * aiguille vers la VUE APPELANTE passée en paramètre. <br/>
	 * <ul>
	 * <li>rafraîchit la liste paginée des objets métier recherchés à chaque
	 * appel.</li>
	 * <li>passe au Model les attributs <strong>'username' , 
	 * 'listeObjetsDansPage'
	 * et 'numerosPagesUserSpring' 
	 * <i>si il y a plus d'une page</i></strong></li>
	 * <li>passe au Model le chemin de la VUE APPELANTE dans l'attribut
	 * <strong>'vueAppelante'</strong>.</li>
	 * <li></li>
	 * </ul>
	 *
	 * @param pUsername : String : identifiant des objets métier inséré 
	 ...
	 * @param pVueAppelante : String : chemin vers la vue appelant la méthode.
	 * @param pModel : org.springframework.ui.Model
	 * 
	 * @return : String : vue appelante.<br/>
	 * 
	 * @throws Exception
	 */
	...
	public String rechercherRapidePagineUserSpring(
			...
			@RequestParam(name = NUMEROPAGEUSERSPRING, defaultValue = "0") final int pNumeroPage,
			@RequestParam(name = SIZEUSERSPRING, defaultValue = DEFAULT_SIZE_PAGE) final int pTaillePage,
		...
		/* rafraîchit la liste paginée des objets métier 
		 * recherchés à chaque appel. */
		this.rafraichirListeRechercheePaginee(
				pUsername, pNumeroPage, pTaillePage, pModel);

		/*
		 * passe au Model le chemin de la VUE APPELANTE dans l'attribut 
		 * 'vueAppelante'.
		 */
	 ...
	 * <code>"/metier/userspring/userspring.html"</code> à chaque lancement de
	 * l'action <code>"/UserSpringController/versPageUserSpring"</code>.<br/>
	 * La path "/UserSpringController" est déjà indiqué dans l'annotation
	 * RequestMapping au dessus de la classe.<br/>
	 * Fournit à la VUE la liste des objets métier 
	 * déjà stockés rafraîchie à chaque appel.
	 * <ul>
	 ...
	 * dans le stockage auprès
	 * de la <code>org.springframework.data.domain.Page</code>.</li>
	 * <li>passe le tableau des numéros (0-based) des pages 
	 * de Users au Model dans
	 * l'attribut <strong>'numerosPagesUserSpring'</strong> 
	 * <i>si il y a plus d'une page</i>.</li>
	 * <li>passe la liste des objets métier dans la Page au Model 
	 * dans l'attribut
	 * <strong>'listeObjetsDansPage'</strong>.</li>
	...
	public String versPageUserSpring(@RequestParam(name = NUMEROPAGEUSERSPRING, defaultValue = "0") final int pNumeroPage,
			@RequestParam(name = SIZEUSERSPRING, defaultValue = DEFAULT_SIZE_PAGE) final int pTaillePage, final Model pModel)
			throws Exception {
	 ...
	 * <code>"/metier/userspring/createUserspring.html"
	 * </code> à chaque lancement de
	 * l'action 
	 ...
	 * dans l'attribut
	 * <strong>'listeObjetsDansPage'</strong>.</li>
	 * <li>délègue au SERVICE la tâche de rapatrier tous les rôles dans le
	 * stockage.</li>
	 * <li>passe au Model la liste des roles dans l'attribut
	 * <strong>'roles'</strong>.</li>
	 * <li>aiguille vers la VUE de CREATION des Objets métier.</li>
	 * </ul>
	 * 
	 * @param pUsername   : String : identifiant de l'objet métier à créer (null
	...
	public String versPageCreateUserSpring(@RequestParam(name = USERNAME, required = false) final String pUsername,
			@RequestParam(name = NUMEROPAGEUSERSPRING, defaultValue = "0") final int pNumeroPage,
			@RequestParam(name = SIZEUSERSPRING, defaultValue = DEFAULT_SIZE_PAGE) final int pTaillePage, final Model pModel)
			throws Exception {

		/* rafraîchit la liste paginée des objets métier à chaque appel. */
		this.rafraichirListeUsersPaginee(pUsername, pNumeroPage, pTaillePage, pModel);

		/*
		 * délègue au SERVICE la tâche de rapatrier tous les rôles dans le stockage.
		 */
	...
	/**
	 * Sauvegarde un Objet Métier dans le stockage et aiguille vers la page de
	 * création des Users SPRING
	 * <code>"/metier/userspring/createUserspring.html"</code> à chaque appel de
	 * l'action <code>/UserSpringController/creerUserSpring</code>.
	 * <ul>
	 * <li>Encode le password avec le passwordEncoder.</li>
	 * <li>délègue au SERVICE la tâche de stockage de l'objet métier.</li>
	 * <li>le SERVICE encode le mot de passe avec un BCryptPasswordEncoder().</li>
	 ...
	 * <strong>'username'</strong> (peut être null en cas d'Exception).</li>
	 * <li>passe le résultat de la création au Model dans l'attribut
	 * <strong>'reponseServiceCreation'</strong> (boolean).</li>
	 * <li>passe le message utilisateur au Model dans l'attribut
	 * <strong>'messageCreation'</strong>.</li>
	...
	public String creerUserSpring(@RequestParam(name = USERNAME, required = true) final String pUsername,
			@RequestParam(name = "password", required = true) final String pPassword,
			@RequestParam(name = NUMEROPAGEUSERSPRING, defaultValue = "0") final int pNumeroPage,
			@RequestParam(name = SIZEUSERSPRING, defaultValue = DEFAULT_SIZE_PAGE) final int pTaillePage, final Model pModel)
			throws Exception {

		UserSpringEntityJPA userSpringStocke = null; // NOPMD by dan on 13/12/19 15:19
		...
		try {
			...
			messageCreation 
				= "le USER a bien été créé dans le stockage : " + pUsername;
			...
			/* constitue dans tous les cas le message utilisateur. */
			messageCreation 
				= "impossible de créer le USER dans le stockage - " 
						+ e.getMessage();
		 ...
		 */
		pModel.addAttribute(
				"reponseServiceCreation", reponseServiceCreation);

		/*
		 * passe le message utilisateur au Model dans l'attribut 'message'.
		 */
	 ...
	 * <code>"/metier/userspring/deleteUserspring.html"</code> à chaque lancement de
	 * l'action <code>"/UserSpringController/deleteUserSpring"</code>.<br/>
	 * La path "/UserSpringController" est déjà indiqué dans l'annotation
	 * RequestMapping au dessus de la classe.<br/>
	 * <ul>
	 * <li>délègue au SERVICE la tâche de destruction de l'objet métier dans le
	 * stockage.</li>
	 * <li>constitue dans tous les cas le message utilisateur.</li>
	 * <li>constitue dans tous les cas la réponse (boolean) du service.</li>
	 * <li>rafraîchit la liste paginée des objets métier recherchés et alimente le
	 * Model avec <strong>'numerosPagesUserSpring', 'listeObjetsDansPage' et 'username' à "" pour que
	 * la recherche rapide retourne tous les utilisateurs en base</strong>.</li>
	...
	public String delete(@RequestParam(name = USERNAME, required = true) final String pUsername
			, final Model pModel)
					throws Exception {
		...
		try {
			...
			/*
			 * délègue au SERVICE la tâche de destruction de l'objet métier dans le
			 * stockage.
			 */
			this.userSpringService.delete(pUsername);

			/* constitue dans tous les cas le message utilisateur. */
			messageDestruction  // NOPMD by dan on 13/12/19 15:24
				= messageDestruction
					+ "L'objet métier a bien été détruit dans le stockage : " // NOPMD by dan on 13/12/19 15:20
					+ pUsername;
			...
			/* constitue dans tous les cas le message utilisateur. */
			messageDestruction  // NOPMD by dan on 13/12/19 15:23
				= messageDestruction 
		...
		/*
		 * rafraîchit la liste paginée des objets métier recherchés et alimente le Model
		 * avec 'numerosPagesUserSpring', 'listeObjetsDansPage' et 'username' à "" pour que la recherche
		 * rapide retourne tous les utilisateurs en base.
		 */
		this.rafraichirListeRechercheePaginee("", 0, DEFAULT_SIZE_PAGE_INT, pModel);

		/*
		 * passe au Model la réponse du service dans l'attribut
		 * 'reponseServiceDestruction'.
		 */
		pModel.addAttribute("reponseServiceDestruction", reponseServiceDestruction);

		/*
		 * passe au Model le message du service dans l'attribut 'messageDestruction'.
		 */
	...
	/**
	 * affecte un (des) ROLE(s) de pRoles <i>présents dans le stockage</i> à un
	 * OBJET METIER pUser à chaque lancement de l'action
	 ...
	 * <li>recherche le RoleEntityJPA dans le stockage pour chaque élément de la
	 * liste pRoles (délègue au DAO).</li>
	 * <li>affecte chaque role stocké de pRolesChoisis à la Collection de roles du
	 * User stocké.</li>
	 * <li>Stocke l'OBJET METIER avec sa nouvelle collection de roles. <strong>Ceci
	 * permet de mettre à jour (update) la table de jointure</strong>.</li>
	 ...
	 * - retourne une SauvegardeImpossibleRunTimeException si le role ne peut être
	 * stocké.<br/>
	 * <br/>
	 *
	 * @param pUsername     : String : ID METIER de l'objet métier auquel affecter
	 *                      les ROLES dans pRoles.
	 * @param pRolesChoisis : List&lt;String&gt; : liste des RoleEntityJPA à
	 *                      affecter au UserSpringEntityJPA.
	 * @param pNumeroPage   : int : numéro (0-based) de la page.
	 * @param pTaillePage   : int : nombre d'enregistrements dans la page.
	 * @param pModel        : org.springframework.ui.Model
	 * 
	...
	public String affecterRolesAUser(@RequestParam(name = USERNAME, required = true) final String pUsername,
			@RequestParam(name = "rolesChoisis", required = true) final List<String> pRolesChoisis,
			@RequestParam(name = NUMEROPAGEUSERSPRING, defaultValue = "0") final int pNumeroPage,
			@RequestParam(name = SIZEUSERSPRING, defaultValue = DEFAULT_SIZE_PAGE) final int pTaillePage, final Model pModel)
			throws Exception {
		...
		final List<RoleEntityJPA> rolesAffectes = new ArrayList<RoleEntityJPA>();

		userSpringEntityJPA = this.userSpringService.loadUserByUsername(pUsername);
		...
		for (final String role : pRolesChoisis) {
			...
			/*
			 * délègue au RoleSERVICE la tâche de trouver les roles dans le stockage.
			 */
			final RoleEntityJPA roleEntityJPA = this.roleService.loadRoleByRolename(role);
		...
		try {
			...
			this.userSpringService.affecterRolesAUser(userSpringEntityJPA, rolesAffectes);
			...
			messageRole = "Impossible d'affecter les rôles au User " + pUsername;
	...
	public Page<UserSpringEntityJPA> listerUsersParPage(
			final int pNumeroPage, final int pTaillePage) throws Exception {
		return null;
	}
	 ...
	 *
	 * @return : List&lt;UserSpringEntityJPA&gt; : liste de tous les User en
	 *         base.<br/>
	 * 
	 * @throws Exception
	 */
	...
	/**
	 * rafraichit une <code>org.springframework.data.domain.Page</code> de numéro
	 * pNumeroPage (0-based) et comportant pTaillePage enregistrements à chaque
	 * appel.
	 * <ul>
	 * <li>délègue au SERVICE la tâche de rafraîchir la Page des objets métier
	 * stockés.</li>
	 * <li>récupère la liste d'objets metier dans la
	 * <code>org.springframework.data.domain.Page</code>.</li>
	 * <li>récupère le nombre total de pages d'objets métier dans le stockage auprès
	 * de la <code>org.springframework.data.domain.Page</code></li>
	 * <li>passe le tableau des numéros (0-based) des pages d'objets métier au Model
	 * dans l'attribut <strong>'numerosPagesUserSpring'</strong> <i>si il y a plus d'une
	 * page</i>.</li>
	 * <li>passe la liste des objets métier dans la Page au Model dans l'attribut
	 * <strong>'listeObjetsDansPage'</strong>.</li>
	 * <li>passe le username au Model dans l'attribut
	 * <strong>'username'</strong>.</li>
	 * </ul>
	 *
	 * @param pUsername   : String : identifiant de l'objet métier saisi dans la
	 *                    zone de création.
	 * @param pNumeroPage : int : numéro (0-based) de la page.
	...
	private void rafraichirListeUsersPaginee(
		...
		int[] numerosPagesUserSpring = null;
		...
		if (page != null) {
			...
			/*
			 * récupère le nombre total de pages d'objets
			 *  métier dans le stockage auprès de
			 * la org.springframework.data.domain.Page.
			 */
			...
			if (nombrePages > 1) {
				...
				// PAGINATION.
				numerosPagesUserSpring = new int[nombrePages];

				/*
				 * passe le tableau des numéros (0-based) 
				 * des pages d'objets métiers au Model
				 * dans l'attribut 'numerosPagesUserSpring' 
				 * si il y a plus d'une page.
				 */
				pModel.addAttribute(
						"numerosPagesUserSpring", numerosPagesUserSpring);
		...
		/*
		 * passe la liste des objets métier dans la Page 
		 * au Model dans l'attribut 'listeObjetsDansPage'.
		 */
	 ...
	 * <ul>
	 * <li>délègue au SERVICE la tâche de rechercher la liste 
	 * paginée des objets métier à partir de pUserName%.</li>
	 * <li>récupère la liste d'objets metier dans la
	 ...
	 * de la <code>org.springframework.data.domain.Page</code>.</li>
	 * <li>passe le tableau des numéros (0-based) des pages 
	 * d'objets metier au Model
	 * dans l'attribut <strong>'numerosPagesUserSpring'</strong> 
	 * <i>si il y a plus d'une
	 * page</i>.</li>
	 * <li>passe la liste des objets métier dans la Page 
	 ...
	 * </ul>
	 *
	 * @param pUsername : String : identifiant des objets métier inséré dans la
	...
	private void rafraichirListeRechercheePaginee(
		...
		/*
		 * délègue au SERVICE la tâche de rechercher la liste 
		 * paginée des objets métier à partir de pUserName%.
		 */
		final Page<UserSpringEntityJPA> page 
		...
		if (page != null) {
			...
			if (nombrePages > 1) {
				...
				/*
				 * passe le tableau des numéros (0-based) 
				 * des pages d'objets metier au Model
				 * dans l'attribut 'numerosPagesUserSpring' 
				 * si il y a plus d'une page.
				 */
				pModel.addAttribute(
