...
public class MaBanqueService implements IMaBanqueService {
	...
	public AbstractCompteEntityJPA consulterCompte(
		...
		/* throw une IdentifiantManquantRunTimeException 
		 * si pCodeCompte n'est pas renseigné (Blank).*/
		...
		try {
			...
			compte = (AbstractCompteEntityJPA) 
					Hibernate.unproxy(this.compteDao.getOne(pCodeCompte));
			...
			if (compte == null) {
				...
				/* throw une ObjetManquantRunTimeException si l'objet 
				 * n'est pas trouvé ans le stockage. */
	...
	@Override
	public void verser(
			final String pCodeCompte, final double pMontant) 
						throws Exception {
		
		final AbstractCompteEntityJPA compte 
			= this.consulterCompte(pCodeCompte);
		
		/* throw une ObjetManquantRunTimeException 
		...
		/* throw une MauvaisParametreRunTimeException 
	...
	public void retirer(
		...
		/* nécessite de faire Hibernate.unproxy(compte) pour connaitre l'instance de compte. */
		Object unproxiedEntity = Hibernate.unproxy(compte);
		
		if (unproxiedEntity instanceof CompteCourantEntityJPA) {
			
			final CompteCourantEntityJPA compteCourantCaste 
				= (CompteCourantEntityJPA) unproxiedEntity;
			
			if (nouveauSolde >= -compteCourantCaste.getDecouvertAutorise()) {
				
				/* Crée et stocke un retrait. */
				final RetraitEntityJPA retrait 
					= new RetraitEntityJPA(
							localDateTimeNow, pMontant, compteCourantCaste);
				...
				final String message 
				= "le solde de votre compte courant et votre facilité de caisse "
						+ "ne permettent pas ce retrait (votre solde atteindrait " 
						+ nouveauSolde + ")";
				
				/* throw une MauvaisParametreRunTimeException 
				 * si le retrait excéderait les facilités de caisse. */
				throw new MauvaisParametreRunTimeException(message);
			}
			
		} else if (unproxiedEntity instanceof CompteEpargneEntityJPA) {
			
			final CompteEpargneEntityJPA compteEpargneCaste 
				= (CompteEpargneEntityJPA) unproxiedEntity;
			...
			if (nouveauSolde >=0) {
				...
				final RetraitEntityJPA retrait 
					= new RetraitEntityJPA(
							localDateTimeNow, pMontant, compteEpargneCaste);
				...
				final String message 
				= "le solde de votre compte d'épargne "
						+ "ne permet pas ce retrait (votre solde atteindrait " 
						+ nouveauSolde + ")";
	...
	@Override
	public void virer(
			final String pCodeCompteSource
				, final String pCodeCompteDestinataire
					, final double pMontant) throws Exception {
		
		
		
		final AbstractCompteEntityJPA compteSource 
			= this.consulterCompte(pCodeCompteSource);
		
		final AbstractCompteEntityJPA compteDestinataire 
			= this.consulterCompte(pCodeCompteDestinataire);
		
		/* throw une ObjetManquantRunTimeException 
		 * si le compte Source est introuvable. */
		...
		/* throw une ObjetManquantRunTimeException 
		 * si le compte Destinataire est introuvable. */
		...
		/* throw une MauvaisParametreRunTimeException 
		 * si les 2 comptes sont identiques. */
		...
		/* Retire pMontant sur le compte Source 
		 * et throw une MauvaisParametreRunTimeException 
		 * si le retrait excéderait les facilités de caisse. */
	 ...
	 */
	@Override
	public Page<AbstractOperationEntityJPA> listerOperationsParPage(
			final String pCodeCompte
				, final int pNumeroPage
					, final int pTaillePage) {
		
		final Page<AbstractOperationEntityJPA> page 
			= this.operationDao.listerOperationsParPage(
					pCodeCompte, PageRequest.of(pNumeroPage, pTaillePage));
